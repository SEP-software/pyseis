...
class WaveEquation(abc.ABC, Operator.Operator):
  ...
  def setup_sep_par(self, fd_param):
    ...
    self.sep_param = self.make_sep_par(sep_param_dict)

  def make_sep_par(self, dict):
  ...
  def setup_wave_prop_operator(self, data_sep, model_sep, sep_par, src_devices,
  ...
  def calc_subsampling(self, model, d_t, model_sampling):
    raise NotImplementedError(
        'calc_subsampling not overwritten by WaveEquation child class')

  @abc.abstractmethod
  def pad_model(self, model, model_sampling, model_padding):
    raise NotImplementedError(
        'pad_model not overwritten by WaveEquation child class')
